...
| 分层 | 数据源 | 覆盖范围 | 调用方式 |
|---|---|---|---|
| S0 权威原始源 | 金监总局、证监会、央行、交易所、行业协会、公司公告 | 监管政策、行政处罚、公告、产品/信息披露 | 官网/RSS/结构化查询 |
| S1 结构化数据源 | **neodata-financial-search**、**westock-data** | 金融保险政策、个股公告、研报评级、资金流向 | Skill/CLI |
| S2 专业财经媒体 | 财新、华尔街见闻、第一财经、财联社深度 | 行业动态、宏观市场、深度报道 | RSS feed（镜像站） |
| S3 快讯/观点线索 | 36氪、财联社电报、WebBridge 扩展源 | 快讯、观点、保险科技动态 | RSS/WebBridge |

### 数据源选择规则

1. **默认先走第一优先**（neodata-financial-search），覆盖面最广、即问即答
2. **需要结构化数据时**（个股公告、资金流向、龙虎榜、研报评级）切 westock-data
3. **需要特定官网原文时**（金监总局法规原文、证监会公告原文）优先找 S0 原始出处
4. **上述都不够时**才走 WebBridge 补充
5. **永远不要凭训练数据脑补**——用户问的是"现在的金融保险行业事实"

## 从业价值评分

面向保险规划师、私募基金经理时，不只按热度排序，而是按"这条信息是否值得从业者今天关注"评分。总分 100：

| 维度 | 分值 | 判断口径 |
|---|---:|---|
| 行业相关性 | 25 | 是否直接涉及保险、基金、资管、银行、监管、财富管理 |
| 来源权威性 | 20 | S0 原始源最高，S1/S2 次之，S3 只作线索 |
| 业务影响 | 20 | 是否影响产品销售、客户资产配置、投研判断、合规风险 |
| 时效性 | 15 | 6 小时内最高，24 小时内较高，超过 3 天递减 |
| 内容深度 | 10 | 是否有政策条文、公告原文、数据、机构观点，而不是一句快讯 |
| 可行动性 | 10 | 是否能转化为客户沟通、投研关注、产品提醒、风险提示 |

输出精选内容时，优先解释高分原因，例如"官方来源 / 影响险资配置 / 可转为客户沟通"。S3 快讯源单条最高不超过 75 分；监管、处罚、法规、批复类媒体转述最高不超过 82 分，优先补原始出处。
